...
/// A token and its half-open UTF-8 byte range in the input SQL.
...
impl TokenWithRange {
    /// Creates a token with its half-open UTF-8 byte range.
...
impl<'a> Tokenizer<'a> {
    ...
    /// Tokenizes the input and includes each token's half-open UTF-8 byte range.
